...
export async function sendLoginAlert(userEmail) {
    try {
        const toEmail = String(userEmail || "").trim().toLowerCase();
        console.log(`Preparing to send login alert to: ${toEmail}`);
        ...
        const response = await resend.emails.send({
            ...
            html: `
                <div style="font-family: Arial, sans-serif; padding: 20px; color: #212529; background-color: #F8F9FA;">
                    ...
                    <p>Hello,</p>
                    <p>We detected a new successful login to your BuyNova account using the email: <strong>${toEmail}</strong>.</p>
        ...
        console.log(`Login alert email cleanly dispatched to: ${toEmail}`, response);
        return response;
    } catch (err) {
